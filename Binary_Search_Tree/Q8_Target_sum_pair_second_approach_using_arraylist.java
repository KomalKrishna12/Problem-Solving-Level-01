...
public class Q8_Target_sum_pair_second_approach_using_arraylist {
  ...
  // firstly using inorder traversal add all node in array list and rest operation
  // on that list
  ...
  // third approach using iterative approach
  static class ITPair {
    int state;
    Node node;
  ...
  public static Node getNormalOrder(Stack<ITPair> stack) {
    while (stack.size() > 0) {
      ITPair pair = stack.peek();
      if (pair.state == 0) {
        if (pair.node.left != null) {
          stack.push(new ITPair(pair.node.left, 0));
        }
        pair.state++;
      } else if (pair.state == 1) {
        if (pair.node.right != null) {
          stack.push(new ITPair(pair.node.right, 0));
        }
        pair.state++;
        return pair.node;
      } else {
  ...
  public static Node getReverseOrder(Stack<ITPair> stack) {
    while (stack.size() > 0) {
      ITPair pair = stack.peek();
      if (pair.state == 0) {
        if (pair.node.right != null) {
  ...
  public static void bestapproach(Node node, int tar) {
    Stack<ITPair> sleft = new Stack<>();
    Stack<ITPair> sright = new Stack<>();
    sleft.push(new ITPair(node, 0));
    sright.push(new ITPair(node, 0));
    Node left = getNormalOrder(sleft);
    Node right = getReverseOrder(sright);
    while (left.data < right.data) {
      if (left.data + right.data > tar) {
        right = getReverseOrder(sright);
      } else if (left.data + right.data < tar) {
        left = getNormalOrder(sleft);
      } else {
