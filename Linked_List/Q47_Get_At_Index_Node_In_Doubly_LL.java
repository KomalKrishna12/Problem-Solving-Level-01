...
public class Q47_Get_At_Index_Node_In_Doubly_LL {
    public static class DoublyLinkedList {
        ...
        // this function will return the node which is at index
        // create a curr node which is pointing to head now
        // create a while loop till index > 0 move curr = curr.next
        // when index become 0 return curr that will be our node
        public Node getAtNode(int index) {
            Node curr = this.head;
            while(index-- > 0) curr = curr.next;
            return curr;
        }
    
        // this function will return data of that particular index
        public int getAt(int index) {
            // if list is empty then ListIsEmptyException() method will be called and return -1
            // if list is empty then indexIsInvalidException() method will be called and return -1
            // this is taking index, starting index which is 0 and last index will be size-1 
            // becoz we start indexing from 0 
            if(ListIsEmptyException()) return -1;
            else if(indexIsInvalidException(index, 0, this.size - 1)) return -1;
